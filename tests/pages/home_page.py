...
class HomePage:
    ...
    def open_category(self, wait, cat_num:int):
        if cat_num > 1 and cat_num < 4:
            try:
                wait.until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, "div[class='list-group'] a:nth-of-type("
                                            + str(cat_num) +")"))
                                            ).click()
    ...
    def open_product(self, wait, name:str):
        try:
            wait.until(
            EC.element_to_be_clickable((By.XPATH, "//a[contains(text(), '"+ name +"')]"))
            ).click()
        except exceptions.ElementNotInteractableException as e:
    ...
    def get_search_products(self, wait, prod_name):
        try:
            ...
            EC.visibility_of_element_located((By.XPATH, "//a[contains(text(), '"+ prod_name +"')]")))
